...
fn setupInstructionInMemory(chip8: *Chip8, address: u16, instruction: u16) void {
    chip8.memory.set_addr(address, @truncate(instruction >> 8));
    chip8.memory.set_addr(address + 1, @truncate(instruction & 0xFF));
}
...
test "Chip8 reset" {
    ...
    try testing.expect(chip8.I_reg == 0);
    try testing.expect(chip8.last_instruction.len == 0);
    try testing.expect(chip8.stack.stack.items.len == 0);
...
test "Chip8 load program" {
    ...
    chip8.load_program(&program, program_name);
    ...
    for (0..program.len) |i| {
        try testing.expectEqual(chip8.memory.load_addr(0x200 + @as(u16, @intCast(i))), program[i]);
    }
}

// --- Decode and Execute Tests ---

test "decode_and_execute - 0x00E0 - CLS" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    ...
    chip8.decode_and_execute(0x00E0);
...
test "decode_and_execute - 0x00EE - RET" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.stack.push(0x0500);
    chip8.decode_and_execute(0x00EE);
...
test "decode_and_execute - 0x1NNN - JP addr" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.decode_and_execute(0x1ABC);

    try testing.expectEqual(chip8.PC, 0x0ABC);
    try testing.expectEqualStrings(chip8.last_instruction, "jump: 0x1NNN");
}

test "decode_and_execute - 0x2NNN - CALL addr" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    chip8.PC = 0x0250;

    chip8.decode_and_execute(0x2300);
...
test "decode_and_execute - 0x4XNN - SNE Vx, byte - Skip" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V2 = 0x12;
    chip8.decode_and_execute(0x4210);

    try testing.expectEqual(chip8.PC, 0x0200 + 2);
    try testing.expectEqualStrings(chip8.last_instruction, "Skib if VX != NN: 0x4XNN");
}

test "decode_and_execute - 0x4XNN - SNE Vx, byte - No Skip" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
...
test "decode_and_execute - 0x5XY0 - SE Vx, Vy - Skip" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V3 = 0x20;
    chip8.register.V4 = 0x20;
    chip8.decode_and_execute(0x5340);

    try testing.expectEqual(chip8.PC, 0x0200 + 2);
    try testing.expectEqualStrings(chip8.last_instruction, "Skip if VX == VY: 0x5XY0");
}

test "decode_and_execute - 0x5XY0 - SE Vx, Vy - No Skip" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
...
test "decode_and_execute - 0x9XY0 - SNE Vx, Vy - Skip" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V5 = 0x30;
    chip8.register.V6 = 0x31;
    chip8.decode_and_execute(0x9560);

    try testing.expectEqual(chip8.PC, 0x0200 + 2);
    try testing.expectEqualStrings(chip8.last_instruction, "skip if VX != VY: 0x9XY0");
}

test "decode_and_execute - 0x9XY0 - SNE Vx, Vy - No Skip" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
...
test "decode_and_execute - 0x6XNN - LD Vx, byte" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.decode_and_execute(0x6742);
...
test "decode_and_execute - 0x7XNN - ADD Vx, byte - No Overflow" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V8 = 0x10;
    chip8.decode_and_execute(0x7805);
...
test "decode_and_execute - 0x8XY1 - OR Vx, Vy - VF cleared" {
    ...
    chip8.decode_and_execute(0x8CD1);
...
test "decode_and_execute - 0x8XY2 - AND Vx, Vy - VF cleared" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    g.clear_VF_after_logic_instructions = true;
    ...
    chip8.decode_and_execute(0x8AB2);
...
test "decode_and_execute - 0x8XY3 - XOR Vx, Vy - VF cleared" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    g.clear_VF_after_logic_instructions = true;
    ...
    chip8.decode_and_execute(0x8013);
...
test "decode_and_execute - 0x8XY4 - ADD Vx, Vy - No Overflow" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V2 = 0x10;
    chip8.register.V3 = 0x20;
    chip8.decode_and_execute(0x8234);
...
test "decode_and_execute - 0x8XY4 - ADD Vx, Vy - Overflow" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
...
test "decode_and_execute - 0x8XY5 - SUB Vx, Vy - No Borrow" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V4 = 0x30;
    chip8.register.V5 = 0x10;
    chip8.decode_and_execute(0x8455);
...
test "decode_and_execute - 0x8XY5 - SUB Vx, Vy - Borrow" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
...
test "decode_and_execute - 0x8XY7 - SUBN Vx, Vy - No Borrow" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V6 = 0x10;
    chip8.register.V7 = 0x30;
    chip8.decode_and_execute(0x8677);
...
test "decode_and_execute - 0x8XY7 - SUBN Vx, Vy - Borrow" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
...
test "decode_and_execute - 0x8XY6 - SHR Vx - VF = LSB, VX = VX >> 1" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    g.set_VX_to_VY_before_shift = false;

    chip8.register.V8 = 0b10101011;
    chip8.decode_and_execute(0x8896);
...
test "decode_and_execute - 0x8XYE - SHL Vx - VF = MSB, VX = VX << 1" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    g.set_VX_to_VY_before_shift = false;

    chip8.register.VA = 0b10101011;
    chip8.decode_and_execute(0x8AEE);
...
test "decode_and_execute - 0xA000 - LD I, addr" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.decode_and_execute(0xA123);

    try testing.expectEqual(chip8.I_reg, 0x123);
    try testing.expectEqualStrings(chip8.last_instruction, "Set I register: 0xANNN");
}

test "decode_and_execute - 0xB000 - JP V0, addr - treat_jump_as_BNNN = true" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    g.treat_jump_as_BNNN = true;

    chip8.register.V0 = 0x10;
    chip8.decode_and_execute(0xB200);
...
test "decode_and_execute - 0xB000 - JP Vx, addr - treat_jump_as_BNNN = false" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    g.treat_jump_as_BNNN = false;

    chip8.register.V3 = 0x20;
    chip8.decode_and_execute(0xB300);
...
test "decode_and_execute - 0xC000 - RND Vx, byte" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.decode_and_execute(0xC4FF);
...
test "decode_and_execute - 0xDXYN - DRW Vx, Vy, nibble - No collision" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    const sprite = [_]u8{ 0xF0, 0x80 };
    chip8.I_reg = 0x200;
    chip8.memory.load_program(&sprite, "test_sprite");

    chip8.register.V0 = 2;
    chip8.register.V1 = 3;
    chip8.register.VF = 1; // should be reset to 0

    chip8.decode_and_execute(0xD012);
...
test "decode_and_execute - 0xDXYN - DRW Vx, Vy, nibble - Collision" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    ...
    chip8.decode_and_execute(0xD011);

    try testing.expectEqual(chip8.register.VF, 1);
    try testing.expectEqualStrings(chip8.last_instruction, "Draw screen: 0xDXYN");
}

test "decode_and_execute - 0xF007 - LD Vx, DT" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.delay_timer.set(0x2A);
    chip8.decode_and_execute(0xF207);
...
test "decode_and_execute - 0xF015 - LD DT, Vx" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V3 = 0x3B;
    chip8.decode_and_execute(0xF315);
...
test "decode_and_execute - 0xF018 - LD ST, Vx" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V4 = 0x4C;
    chip8.decode_and_execute(0xF418);
...
test "decode_and_execute - 0xF029 - LD F, Vx" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V5 = 0x05;
    chip8.decode_and_execute(0xF529);
...
test "decode_and_execute - 0xF033 - LD B, Vx" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.register.V6 = 123;
    chip8.I_reg = 0x500;
    chip8.decode_and_execute(0xF633);

    try testing.expectEqual(chip8.memory.load_addr(0x500), 1);
    try testing.expectEqual(chip8.memory.load_addr(0x501), 2);
    try testing.expectEqual(chip8.memory.load_addr(0x502), 3);
    try testing.expectEqualStrings(chip8.last_instruction, "Store VX as decimal in memory: 0xFX33");
}

test "decode_and_execute - 0xF055 - LD [I], Vx - No increment I" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    ...
    chip8.I_reg = 0x600;
    chip8.decode_and_execute(0xF255);

    try testing.expectEqual(chip8.memory.load_addr(0x600), 0x10);
    try testing.expectEqual(chip8.memory.load_addr(0x601), 0x20);
    try testing.expectEqual(chip8.memory.load_addr(0x602), 0x30);
...
test "decode_and_execute - 0xF055 - LD [I], Vx - Increment I" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
...
test "decode_and_execute - 0xF065 - LD Vx, [I] - No Increment I" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    g.increment_I_on_store_load = false;

    chip8.I_reg = 0x700;
    chip8.memory.set_addr(0x700, 0xAA);
    chip8.memory.set_addr(0x701, 0xBB);
    chip8.memory.set_addr(0x702, 0xCC);
    chip8.decode_and_execute(0xF265);
...
test "decode_and_execute - 0xF065 - LD Vx, [I] - Increment I" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    g.increment_I_on_store_load = true;
...
test "decode_and_execute - 0xF075 - LD R, Vx" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    ...
    chip8.decode_and_execute(0xF275);
...
test "decode_and_execute - 0xF085 - LD Vx, R" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
    ...
    chip8.decode_and_execute(0xF285);
...
test "decode_and_execute - 0xF01E - ADD I, Vx - No Overflow" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();

    chip8.I_reg = 0x0100;
    chip8.register.V7 = 0x0020;
    chip8.decode_and_execute(0xF71E);
...
test "decode_and_execute - 0xF01E - ADD I, Vx - Overflow" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
...
test "execute_one_instruction - Fetches and executes instruction" {
    var chip8 = try Chip8.init(std.heap.page_allocator);
    defer chip8.deinit();
