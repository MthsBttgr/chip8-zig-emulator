...
test "Memory loadProgram - loads program and font, stores program name" {
    var mem = try Memory.init(std.heap.page_allocator);
    defer mem.deinit();
    ...
    mem.loadProgram(program, program_name);

    // Check if program is loaded at 0x200
    try testing.expectEqual(mem.loadAddr(0x200), 0x10);
    try testing.expectEqual(mem.loadAddr(0x201), 0x20);
    try testing.expectEqual(mem.loadAddr(0x202), 0x30);

    // Check if font is loaded (basic check, first font byte)
    try testing.expectEqual(mem.loadAddr(mem.font_start_addr), 0xF0);
...
test "Memory set_addr and loadAddr - set and get memory at address" {
    var mem = try Memory.init(std.heap.page_allocator);
    defer mem.deinit();
    ...
    mem.setAddr(address, data);
    const loaded_data = mem.loadAddr(address);
